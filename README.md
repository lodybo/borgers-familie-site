...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create borgers-site-f22c
  fly apps create borgers-site-f22c-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app borgers-site-f22c
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app borgers-site-f22c-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'borgers-site-f22c-staging' does not match app name in config file 'borgers-site-f22c'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name borgers-site-f22c-db
  fly postgres attach --app borgers-site-f22c borgers-site-f22c-db

  fly postgres create --name borgers-site-f22c-staging-db
  fly postgres attach --app borgers-site-f22c-staging borgers-site-f22c-staging-db
  ```
